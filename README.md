...
**Core capabilities:**
- BM25/INSTR hybrid search — **94.4% Hit@5** on Chinese (no AI components required)
...
**核心能力：**
- BM25/INSTR 搜尋，中文 **94.4% Hit@5**（無需 AI 組件）
...
| **Chinese Hit@5** / 中文搜尋命中率 | ~60% (est.) | ~75% (est.) | **94.4%** (measured, no AI required) |
...
MemPalace is designed for English; MemOcean is designed for Chinese work scenarios. For context: MemPalace achieves 96.6% on LongMemEval in **raw verbatim mode** (no compression), but drops to **84.2% with AAAK compression enabled** (−12.4pp). MemOcean CLSC Sonar achieves 94.4% **with compression active** — 10.2pp above MemPalace's compressed mode.

Benchmarks run 2026-04-17 with pure BM25/INSTR (zero AI components):

| Benchmark | Language | Hit@5 | Notes |
|---|---|---|---|
| Internal | Chinese (mixed) | **94.4%** | 300-question internal corpus (expanded from 240) |
...
### 2026-04-17
- **Final benchmark run**: Internal corpus expanded to 300 questions (was 240); benchmark script migrated from `closet` to `radar` table (586 entries). New results: internal **94.4%** Hit@5 / DRCD 91.9% / CMRC 93.3% / BEIR 70.7%. KG query: 100% (19/19).
