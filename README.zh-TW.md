...
| **中文搜尋命中率** | ~60% Hit@5（估算）| ~75% Hit@5（估算）| **86.2% Hit@5**（實測）|
...
| **Token 精簡率** | ~91%（AAAK）| N/A | **87%（CLSC Sonar）** |
| **開源授權** | MIT | MIT | MIT |
...
檢索採用三段式 hybrid recall，全程 API 呼叫、不依賴本地 GPU：

```
keyword（FTS5 BM25）
  +                     ──→ 合併 top-K 候選 ──→ Haiku LLM reranker ──→ 精排結果
embedding KNN（API）
```

- **keyword**：FTS5 trigram + BM25，精確匹配已知 entity，<10ms
- **embedding KNN**：語意向量近鄰搜尋，補推斷詞 miss，透過 embedding API 呼叫，不需本地 GPU
- **Haiku reranker**：輕量 LLM 精排，在工作場景語境中排序穩定，比純向量 cosine 更貼合中文語境

---

## CLSC 中文 Sonar 萃取引擎

**CLSC**（Chinese Lossy Skeleton Codec）是 MemOcean 的核心引擎，fork 自 MemPalace 的 AAAK skeleton 格式（MemPalace 稱為 Closet），針對中文場景重寫了整條 NER + 搜尋 pipeline。
...
MemPalace 為英文場景設計，MemOcean 為中文工作場景設計。各自在擅長的語言上驗證：

| | MemPalace AAAK skeleton | MemOcean Hybrid+Haiku |
|---|---|---|
| Benchmark | LongMemEval（英文） | MADial-Bench（中文） |
| Hit@5 / R@5 | 84.2% | **87.5%** |
| Hit@1 | N/A | **68.1%** |

MemOcean 在中文場景超越 MemPalace 英文 sonar 模式（MemPalace 稱為 skeleton）**+2.0pp**（Hit@5）。

交叉驗證（英文 LongMemEval）：MemOcean Seabed+BM25 **90.5%** R@5 vs MemPalace AAAK **84.2%**（+6.3pp）。

大規模壓力測試（CRUD-RAG）：中文新聞 20K 文檔，**99%** hit rate，**14ms** 延遲。
...
- **Benchmark 86.2% Hit@5**：Hybrid+Haiku reranker 穩定 baseline（LongMemEval 90.5% / MADial 87.5% / CRUD-RAG 99%）。
