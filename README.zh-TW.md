...
MemPalace 為英文場景設計，MemOcean 為中文工作場景設計。以下為 2026-04-16 純 BM25/INSTR（零 AI 組件）實測結果：
...
| CLSC A/B | — | tag vs no-tag **+1.9pp** at Hit@5 | 確認 tag 格式對召回有貢獻 |

CLSC token 壓縮：1,716,211 原始 tokens → 129,529 skeleton tokens = **13x 壓縮（92.5% 減少）**。每筆中位數比率：18.9%。
...
- **Benchmark 更新**：internal Hit@5=92.9%；DRCD 繁中=91.9%；CMRC 簡中=93.3%；BEIR SciFact 英文=70.7%。
- **CLSC 壓縮數據確認**：1,716,211 原始 tokens → 129,529 skeleton tokens，13x 壓縮（92.5% 減少），每筆中位數 18.9%。
