...
MemPalace 為英文場景設計，我們為中文場景設計。各自在擅長的語言上驗證：

| | MemPalace（英文） | MemOcean（中文） |
|---|---|---|
| Benchmark | LongMemEval (500 題) | MADial-Bench (160 段對話) |
| 指標 | Recall@5 | Hit@10 |
| Raw / Seabed | 96.6% | — |
| Skeleton / Sonar + BM25 | 84.2% | 79.4% |

我們在英文 LongMemEval 上也跑了一輪作為交叉驗證：Seabed + FTS5 BM25 拿到 90.5% R@5，超越 MemPalace AAAK skeleton（84.2%）+6.3pp。

中文場景的提升空間在語意檢索（embedding reranker），已在路線圖中。
