...
**英文：LongMemEval（500 題，Recall@5）**

| 系統 | 模式 | R@5 |
|------|------|-----|
| MemPalace | raw（長上下文） | 96.6% |
| **MemOcean** | **Seabed + FTS5 BM25** | **90.5%** |
| MemPalace | AAAK skeleton | 84.2% |

MemOcean 的 BM25 檢索在同一 benchmark 上超越 MemPalace 的 AAAK skeleton +6.3pp，接近 raw 長上下文的效果。

**中文：MADial-Bench（160 段對話，Hit@K）**

| 方法 | Hit@1 | Hit@5 | Hit@10 |
|------|-------|-------|--------|
| Embedding（最佳） | 64.4% | 90.0% | 96.2% |
| **MemOcean keyword** | **28.1%** | **65.6%** | **79.4%** |

中文場景下關鍵字檢索的 Hit@10 達 79.4%，作為第一階段 retriever 足夠。精確度的提升空間在於加入 embedding reranker（路線圖 C/D）。
