#!/usr/bin/env python3
"""
onboarding.py — CLSC v0.6 first-run setup.

Asks the user:
  1. How they're using CLSC (work / personal / combo)
...
Seeds the entity_registry with confirmed data so CLSC knows your world
from minute one — before a single session is indexed.

Usage:
    python3 onboarding.py
    or: python3 -m clsc.v0.6.onboarding
"""

from pathlib import Path

# Forked for CLSC v0.6 HanCloset — standalone, no upstream package dependency.
# EntityRegistry and entity_detector stubs provided below.

class EntityRegistry:
    """Minimal stub for standalone use."""
    def __init__(self, path=None):
        self.entities = {}
    def add(self, name, aliases=None, category=None):
        self.entities[name] = {'aliases': aliases or [], 'category': category}
    def save(self, path):
        import json
        Path(path).write_text(json.dumps(self.entities, ensure_ascii=False, indent=2))

def detect_entities(text):
    return []

def scan_for_detection(paths):
    return []
...
def _ask_mode() -> str:
    _header("Welcome to CLSC v0.6")
    print("""
  CLSC is a shorthand language system. To work well, it needs to know
...
def _ask_projects(mode: str) -> list:
    ...
    _hr()
    print("""
  What are your main projects? (These help CLSC distinguish project
...
def _warn_ambiguous(people: list) -> list:
    ...
    """
    # from upstream.entity_registry import COMMON_ENGLISH_WORDS
...
def _generate_clsc_bootstrap(
    people: list, projects: list, wings: list, mode: str, config_dir: Path = None
):
    """
    Generate CLSC entity registry + critical facts bootstrap from onboarding data.
    These files teach the AI about the user's world from session one.
    """
    clsc_dir = Path(config_dir) if config_dir else Path.home() / ".clsc"
    clsc_dir.mkdir(parents=True, exist_ok=True)

    # Build CLSC entity codes (first 3 letters of name, uppercase)
    entity_codes = {}
    for p in people:
        name = p["name"]
        code = name[:3].upper()
    ...
    # CLSC entity registry
    registry_lines = [
        "# CLSC Entity Registry",
        "# Auto-generated by clsc init. Update as needed.",
        "",
        "## People",
    ]
    for p in people:
    ...
    (clsc_dir / "clsc_entities.md").write_text("\n".join(registry_lines))
    ...
    (clsc_dir / "critical_facts.md").write_text("\n".join(facts_lines))
...
def run_onboarding(
  ...
  CLSC will check the context before treating them as person names.
...
""")
    ...
    # Generate CLSC entity registry + critical facts bootstrap
    _generate_clsc_bootstrap(people, projects, wings, mode, config_dir)
    ...
    print("\n  CLSC entity registry: ~/.clsc/clsc_entities.md")
    print("  Critical facts bootstrap: ~/.clsc/critical_facts.md")
